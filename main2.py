...
OUTPUT_DIR = "output_results22" # Changed from "output" to be more specific
BERT_CHECKPOINT = "my_finetuned_minilm_model" # Keep the existing BERT model

# --- NEW: Similarity Thresholds and Window Sizes ---
...
def rank_sections_and_extract_subsections(
    ...
    subsection_block_window_size # New parameter
):
    """
    ...
    for rank, section_info in enumerate(sections_to_process): # Iterate over the filtered list
        ...
        if best_block_info:
            ...
            refined_text_entry = {
                ...
                "refined_text_similarity": float(max_block_similarity) # Similarity of the single best block
            }
...
def main_round1b_processing(input_dir, output_dir, persona, job_to_be_done):
    ...
    final_extracted_sections, all_subsection_analysis = rank_sections_and_extract_subsections(
        all_docs_sections_for_ranking, persona, job_to_be_done, MIN_SIMILARITY_THRESHOLD, SUBSECTION_BLOCK_WINDOW_SIZE # Pass the new parameter
    )
