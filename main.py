    ...
    def process_node_and_children(node_list):
        for i, node in enumerate(node_list):
            ...
            sections_with_content.append({
                ...
                "sub_blocks": content_only_blocks.to_dict(orient='records') # Store as list of dicts for refined text
            })
...
def rank_sections_and_extract_subsections(
    ...
    Also extracts the most relevant sentence ('refined text') for sub-section analysis.
    """
    ...
    if persona_embedding is None:
        persona_embedding = np.zeros_like(job_embedding)
    if job_embedding is None:
        job_embedding = np.zeros_like(persona_embedding)

    # If both are None (unlikely with dummy data but good practice)
    if persona_embedding is None and job_embedding is None:
        weighted_query_embedding = np.zeros(bert_model.config.hidden_size)
    elif persona_embedding is None:
        weighted_query_embedding = job_embedding
    elif job_embedding is None:
    ...
    for section in all_document_sections_with_content:
        ...
        similarity_score = calculate_cosine_similarity(query_embedding, section_embedding)
    ...
    # Assign importance_rank and extract refined text for top sections
    for rank, section_info in enumerate(sections_with_scores):
...
def main_round1b_processing(input_dir, output_dir, persona, job_to_be_done):
    ...
    final_extracted_sections, all_subsection_analysis = rank_sections_and_extract_subsections(
        all_docs_sections_for_ranking, persona, job_to_be_done
    )
